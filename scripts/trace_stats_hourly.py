...
Reads traces/run_*.jsonl and prints hourly buckets (UTC) for:
- total calls
- ok/fail
...
def main(argv: list[str]) -> int:
    ...
    for fp in files:
        for e in iter_jsonl(fp):
            ...
            if ts is None:
                continue
            hour_key = ts.strftime("%Y-%m-%d %H:00Z")
            b = buckets[hour_key]
            b["total"] += 1
            if ok_flag(e):
                b["ok"] += 1
    ...
    # Print sorted
    print("hour\ttotal\tok\tfail\ttransport_error\twith_retry")
